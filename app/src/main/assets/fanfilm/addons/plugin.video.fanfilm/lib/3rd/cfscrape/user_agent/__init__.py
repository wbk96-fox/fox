    ...
    def loadUserAgent(self, *args, **kwargs):
        ...
        if self.custom:
            ...
            if self.browser and self.browser not in self.browsers:
                sys.tracebacklimit = 0
                raise RuntimeError(
                    'Sorry "{}" browser is not valid, valid browsers are [{}].'.format(
            ...
            if self.platform not in self.platforms:
                sys.tracebacklimit = 0
                raise RuntimeError(
                    'Sorry the platform "{}" is not valid, valid platforms are [{}]'.format(
            ...
            if not filteredAgents[self.browser]:
                sys.tracebacklimit = 0
                raise RuntimeError(
                    'Sorry "{}" browser was not found with a platform of "{}".'.format(
