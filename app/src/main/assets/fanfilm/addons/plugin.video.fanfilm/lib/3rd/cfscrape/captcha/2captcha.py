    ...
    def checkErrorStatus(response, request_type):
        if response.status_code in [500, 502]:
            raise CaptchaServiceUnavailable(
                "2Captcha: Server Side Error {}".format(response.status_code)
            )
        ...
        try:
            ...
            try:
                ...
                raise CaptchaTimeout(
                    "2Captcha: Captcha solve took to long and also failed reporting the job the job id {}.".format(
                        jobID
                    )
                )

            raise CaptchaTimeout(
                "2Captcha: Captcha solve took to long to execute job id {}, aborting.".format(
                    jobID
                )
            )
