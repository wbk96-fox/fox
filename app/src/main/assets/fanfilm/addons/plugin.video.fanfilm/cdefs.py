...
class InfoDetails(Flag):
    ...
    #: Request media main details like 'release_dates', 'keywords', 'credits', 'alternative_titles', etc. (info request).
    MEDIA_DETAILS = 0x010
...
class StrmFilename(Enum):
    ...
    # No title, Kodi only needs SxxEyy to scrape an episode. Example: S01E07.strm
    BARE = 'bare'
...
ListService: TypeAlias = Literal['local', 'library', 'own', 'trakt', 'tmdb', 'mdblist', 'imdb', 'logs']
...
RatingServiceName = Literal['tmdb', 'trakt', 'mdblist', 'imdb']
...
class const:
    ...
    class dialog:
        ...
        @constdef
        class changelog:
            #: Color of the current addon version header (AARRGGBB).
            version_color: str
