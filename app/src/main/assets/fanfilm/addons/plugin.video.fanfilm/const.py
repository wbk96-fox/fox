...
# example 'rapideo', 'nopremium', 'twojlimit','tb7', 'xt7', 'gb7'
# tb7/xt7/gb7 - long time to scrape
...
# View for "My Movies" / "My TV Shows" root folder (Trakt/TMDB watchlist, favorites, etc.).
const.indexer.my_lists_view = 'sets'
...
# Crew details in details folder (.../info). AGGREGATE_CREDITS fetches aggregate_credits for
# show/season and plain credits for movie/episode (see create_append_to_response()).
...
#: Max concurrent requests when scanning all pages in trakt.tv (independent of
#: threads.count, which is sized for scraper providers, not Trakt's rate limits).
const.trakt.scan.page.workers = 5
...
# Trakt.tv close to token expire time in seconds to refresh access token.
const.trakt.connection.close_to_token_expire = 3600
...
#   - provider – source provider, e.g. 'tb7', 'xt7', 'gb7', 'external', 'wizjacc', etc.
...
#   - RegEx is full mached regular expression, e.g. RegEx('tb7') or R('tb7|xt7|gb7').
...
const.sources.rules = {
    # buy again CM
    SourcePattern(provider=R('tb7|xt7|gb7')): SourceAttribute(menu=('buy', 'play')),
    ...
    # SourcePattern(provider='ecample_of_only_kodi_22_scraper', kodi=Expr('< 22')): False,
}
...
# Use FanFilm's custom resolvers (lib/resolvers/) for hosts where we maintain
# our own implementation. When False every URL goes straight to upstream
# script.module.resolveurl. See lib/resolvers/__init__.py.
const.sources.local_resolvers = True
...
# tb7/xt7/gb7 use similarity.
const.sources.xtb7.similarity_check = False

# tb7/xt7/gb7 similarity threshold.
...
# tb7/xt7/gb7 title replacements.
...
# Extra debug info for tb7 / xt7 / gb7 provider.
...
# tb7/xt7/gb7 use two separate login sessions for parallel EN+PL search.
...
const.library.safe_filename = True
...
const.library.service.chunk_sleep = 5

# If True, offer the trakt watchlist in the "pick lists to sync" dialog. Off by default: unlike
# the other services (plain lists, not trackers), trakt drops an item from the watchlist as soon
# as you start watching it, so a watchlist-sourced sync may silently stop covering a show mid-season.
const.library.picker.show_trakt_watchlist = False
...
# Changelog dialog: color of the current addon version (readable on light and dark skins).
const.dialog.changelog.version_color = 'FFD81B60'
...
const.dialog.add_to.lists.default = {
    ...
    L(30630, 'IMDb'): ('imdb:watchlist', 'imdb:user'),
...
const.tune.service.web_server.cookies = {
    # --- services ---
    'www.imdb.com': {  # private lists and modify list and watchlist in IMDB
        'at-main': 'imdb.at-main',
        ':user_agent': 'imdb.user_agent',
        # only present when the FanFilm panel's "Dodaj ... jako imdb.user" button was clicked on a
        # /user/p.xxx/ profile page - appended (never overwrites; imdb.user may hold several users)
        ':imdb_user_id': ('imdb.user', 'append'),
    },
    # --- sources ---
    'cda-hd.cc': {
    ...
    'p.111477.xyz': {  # "Mint" proxy tool used by lib/sources/en/dahmermovies.py
        'cf_clearance': 'dahmer111477.cookies_cf',
        ':user_agent': 'dahmer111477.user_agent',
    },
    'netmirror.gg': {  # not an actual cookie - the bypass OTP code scraped from netmirror.gg/tv
        'otp_code': 'netmirror.otp_code',
    },
...
# control.groupedInfoDialog(): seconds of silence before showing a collapsed notification.
const.tune.notify.debounce = 3.0
# control.groupedInfoDialog(): max seconds to wait since the first call in a burst.
const.tune.notify.max_wait = 15.0
# control.groupedInfoDialog(): min seconds between two notifications (any keys), so they never overlap.
# infoDialog's default on-screen time is 3s (see infoDialog(time=3000)) + 0.5s buffer.
const.tune.notify.queue_gap = 3.5
