...
from lib import DEBUG_LEFTOVER_OBJECTS, MODULES_TO_REMOVE_ON_EXIT # noqa: E402
...
try:
    ...
    fflog(f' [FF] ==================== {sys.modules.keys()} ====================')
    if MODULES_TO_REMOVE_ON_EXIT:
        # remove same modules to free kodi objects (Addons, ListItems, etc.) from memory
        from lib.ff.tricks import remove_modules
        remove_modules(MODULES_TO_REMOVE_ON_EXIT)
    if DEBUG_LEFTOVER_OBJECTS:
        from lib.ff.tricks import dump_leftover_objects
        dump_leftover_objects()
