...
from lib import DEBUG_LEFTOVER_OBJECTS
...
if True:
    from lib.ff import kotools
    kotools.destroy_xmonitor()
    del kotools.KodiMonitor

if DEBUG_LEFTOVER_OBJECTS >= 1:
    import gc, xbmcaddon, xbmcgui, xbmc
    # gc.collect()
    for a in (o for o in gc.get_objects() if isinstance(o, (xbmcaddon.Addon, xbmcgui.ListItem, xbmc.Monitor))):
        try:
            aid = a.getAddonInfo('id')
        except Exception:
            aid = '?'
        fflog(f'[LEFTOVER] Addon id={aid!r}: bt={getattr(a, "_traceback", "?")}')
        for r in gc.get_referrers(a):
            fflog(f'    <- {type(r).__module__}.{type(r).__name__}: {getattr(r, "__name__", "") or "???"}')

fflog(' [SERVICE] finished ---------')
