...
import com.foxtv.app.core.player.DolbyVisionBaseLayerPolicy
import com.foxtv.app.core.player.BitrateAwareLoadControl
import com.foxtv.app.core.player.DolbyVisionConversionConfig
import com.foxtv.app.core.player.DolbyVisionConversionStats
import com.foxtv.app.core.player.DolbyVisionExtractorsFactory
import com.foxtv.app.core.player.DoviBridge
import com.foxtv.app.core.player.LastPlaybackDiagnostics
import com.foxtv.app.core.tracking.TrackingScrobbleAction
import com.foxtv.app.ui.screens.settings.MemoryBudget
...
internal fun PlayerRuntimeController.initializePlayer(
    ...
    scope.launch {
        try {
            ...
            FoxTvExoPlayerPerformanceHelper.updateSettings(playerSettings, context)
            FoxTvExoPlayerPerformanceHelper.enabled = playerSettings.foxTvPerformanceModeEnabled
            ...
            val rawBandwidthMeter = if (FoxTvExoPlayerPerformanceHelper.enabled) {
                FoxTvExoPlayerPerformanceHelper.buildBandwidthMeter(context)
            } else {
            ...
            val loadControl = if (isIptvPlayback) {
                ...
                FoxTvExoPlayerPerformanceHelper.buildIptvLiveLoadControl()
            } else if (playerSettings.foxTvPerformanceModeEnabled) {
                effectiveBackBufferDurationMs = FoxTvExoPlayerPerformanceHelper.backBufferMs
                currentBitrateAwareLoadControl = null
                Log.i(
                    PlayerRuntimeController.TAG,
                    "BUFFER_GATE: engine=exo-native-perf master=on; FoxTvExoPlayerPerformanceHelper.buildLoadControl host=${url.safeHost()}"
                )
                FoxTvExoPlayerPerformanceHelper.buildLoadControl(context)
                ...
                val allocator = androidx.media3.exoplayer.upstream.DefaultAllocator(
                    ...
                    64,
                    playerSettings.foxTvPerformanceModeEnabled
                )
            ...
            if (playerSettings.parallelNetworkEnabled) {
                ...
                mediaSourceFactory.foxTvPerformanceModeEnabled = playerSettings.foxTvPerformanceModeEnabled
            } else {
                // Reset each playback so the factory doesn't keep last stream's state.
                mediaSourceFactory.useParallelConnections = false
                mediaSourceFactory.foxTvPerformanceModeEnabled = false
            }
            ...
            trackSelector = object : DefaultTrackSelector(context, adaptiveTrackSelectionFactory) {
                override fun selectAllTracks(
                    ...
                    Log.d("FoxTvTrackSelector", "selectAllTracks run: streamMime=$streamMime, isHls=$isHls")
                    if (isHls) {
                            ...
                            if (mappedTrackInfo.getRendererType(rendererIndex) == C.TRACK_TYPE_VIDEO) {
                                ...
                                for (groupIndex in 0 until trackGroups.length) {
                                    ...
                                    for (trackIndex in 0 until group.length) {
                                        ...
                                        Log.d("FoxTvTrackSelector", "Evaluating track: id=${format.id}, res=${format.width}x${format.height}, mime=${format.sampleMimeType}, codecs=${format.codecs}, support=${formatSupport}")
                                        if (formatSupport == C.FORMAT_EXCEEDS_CAPABILITIES) {
                                            ...
                                            Log.d("FoxTvTrackSelector", "Conditions for id=${format.id}: isAvcOrHevc=$isAvcOrHevc, isAtMost1080p=$isAtMost1080p, isStandard8Bit=$isStandard8Bit")
                                            if (isAvcOrHevc && isAtMost1080p && isStandard8Bit) {
                                                Log.i("FoxTvTrackSelector", "Upgraded track support to FORMAT_HANDLED for id=${format.id}")
            ...
            com.foxtv.app.core.player.dvmkv.DolbyVisionCompatibility.setHdr10BaseLayerModeActive(isHdr10BaseLayerModeActive)
            ...
            val effectiveAudioOutputChannels = if (bluetoothStereoDownmix) {
                com.foxtv.app.data.local.AudioOutputChannels.CHANNELS_2_0
            } else {
            ...
            _exoPlayer?.apply {
                ...
                addListener(object : Player.Listener {
                    override fun onPlaybackStateChanged(playbackState: Int) {
                        ...
                        _uiState.update {
                            it.copy(
                                isBuffering = if (FoxTvExoPlayerPerformanceHelper.shouldSuppressBufferingUi(
                        ...
                        if (playbackState == Player.STATE_READY) {
                            ...
                            _uiState.value.pendingSeekPosition?.let { position ->
                                seekTo(position)
                                if (FoxTvExoPlayerPerformanceHelper.enabled) {
                            ...
                            if (!FoxTvExoPlayerPerformanceHelper.shouldGuardTrackRebuild() || !hasRenderedFirstFrame) {
            ...
            if (
                maybeAutoSwitchInternalPlayerOnStartupError(
                    detailedError = e.message ?: context.getString(com.foxtv.app.R.string.player_error_initialize_failed),
                    allowEngineFailover = allowEngineFailover
                )
            ) {
                return@launch
            }
            val displayError = e.toDisplayMessage(context, context.getString(com.foxtv.app.R.string.player_error_initialize_failed))
...
private class SubtitleOffsetRenderersFactory(
    ...
    private val audioOutputChannels: com.foxtv.app.data.local.AudioOutputChannels,
...
private fun FfmpegAudioRenderer.applyDownmixSettings(
    ...
    audioOutputChannels: com.foxtv.app.data.local.AudioOutputChannels,
...
private fun PlayerRuntimeController.recordFirstFrameDiagnostics(
    ...
    playerSettings: com.foxtv.app.data.local.PlayerSettings
