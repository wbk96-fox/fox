        ...
        try {
            ...
            _uiState.update {
                it.copy(
                    ...
                    subtitleAutoSyncError = if (parsedCues.isEmpty()) {
                        context.getString(com.foxtv.app.R.string.subtitle_timing_file_no_lines)
                    } else {
                        null
                    },
                    subtitleAutoSyncLoadedTrackKey = selectedTrackKey
            ...
            _uiState.update {
                it.copy(
                    subtitleAutoSyncLoading = false,
                    subtitleAutoSyncCues = emptyList(),
                    subtitleAutoSyncError = e.message ?: context.getString(com.foxtv.app.R.string.subtitle_timing_load_lines_failed),
                    subtitleAutoSyncLoadedTrackKey = selectedTrackKey
                )
            }
        }
...
private fun PlayerRuntimeController.executeSubtitleDownload(url: String, languageHint: String? = null): String {
    ...
    subtitleAutoSyncHttpClient.newCall(requestBuilder.build()).execute().use { response ->
        if (!response.isSuccessful) {
            error(context.getString(com.foxtv.app.R.string.subtitle_download_failed_http, response.code))
        }
        val bodyBytes = response.body?.bytes()
            ?: error(context.getString(com.foxtv.app.R.string.subtitle_download_empty_content))
