...
internal fun PlayerRuntimeController.attachMpvView(view: FoxTvMpvSurfaceView?) {
    ...
    runCatching {
        ...
        val detailedError = it.message ?: context.getString(com.foxtv.app.R.string.player_error_mpv_surface_failed)
        if (
...
internal fun PlayerRuntimeController.initializeMpvPlayer(
    ...
    runCatching {
        setLoadingStatus(
            phase = "mpv_starting",
            message = context.getString(com.foxtv.app.R.string.player_loading_starting),
            showOverlay = true
        )
        performPendingMpvHardRestartIfNeeded(view)
        view.applyHardwareDecodeMode(mpvHardwareDecodeModeSetting)
        ...
        val detailedError = error.message ?: context.getString(com.foxtv.app.R.string.player_error_mpv_playback_failed)
        if (
            maybeAutoSwitchInternalPlayerOnStartupError(
                detailedError = detailedError,
...
private fun PlayerRuntimeController.performPendingMpvHardRestartIfNeeded(view: FoxTvMpvSurfaceView): Boolean {
    ...
    if (isUsingMpvEngine()) {
        ...
        _exoPlayer?.let { player ->
            if (FoxTvExoPlayerPerformanceHelper.enabled) {
                ...
                val inBuffer = isForwardSeek && FoxTvExoPlayerPerformanceHelper.isSeekInBuffer(player, positionMs)
                if (inBuffer) {
                ...
                FoxTvExoPlayerPerformanceHelper.buildScrubbingParams()?.let { params ->
...
internal fun PlayerRuntimeController.scheduleSeekSuppressTimeout() {
    scope.launch {
        delay(FoxTvExoPlayerPerformanceHelper.SEEK_SUPPRESS_TIMEOUT_MS)
        suppressBufferingUiForSeek = false
    }
}
