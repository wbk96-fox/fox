...
import com.foxtv.app.data.local.SubtitleOrganizationMode
import com.foxtv.app.data.local.TrailerSettings
...
class PlaybackSettingsViewModel @Inject constructor(
    ...
    suspend fun setBufferTargetSizeMb(mb: Int) {
        ...
        if (!current.useParallelConnections || current.foxTvPerformanceModeEnabled || current.allowLargeTargetBuffer) {
    ...
    suspend fun resetBufferSettingsToDefaults() {
        ...
        val current = playerSettings.first()
        if (!current.useParallelConnections || current.foxTvPerformanceModeEnabled || current.allowLargeTargetBuffer) return
    ...
    suspend fun setUseParallelConnections(enabled: Boolean) {
        ...
        val current = playerSettings.first()
        if (current.foxTvPerformanceModeEnabled || current.allowLargeTargetBuffer) {
    ...
    suspend fun setFoxTvPerformanceModeEnabled(enabled: Boolean) {
        playerSettingsDataStore.setFoxTvPerformanceModeEnabled(enabled)
    }
}
