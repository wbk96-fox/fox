...
import com.foxtv.app.data.local.StartupSyncPreferences
...
class StartupSyncService @Inject constructor(
    ...
    private val cwEnrichmentCache: com.foxtv.app.data.local.ContinueWatchingEnrichmentCache
) {
    ...
    fun requestRealtimeSurfacePull(profileId: Int, surface: String) {
        ...
        scope.launch {
            ...
            when (surface) {
                ...
                "library" -> pullFoxTvLibrary(profileId)
                "watch_progress" -> {
    ...
    private suspend fun pullPeriodicLibrary(): Boolean {
        ...
        return pullFoxTvLibrary(profileId)
    }
    ...
    private suspend fun pullFoxTvLibrary(profileId: Int): Boolean {
        val isTrackingLibrary = libraryRepository.sourceMode.first() != LibrarySourceMode.LOCAL
        if (isTrackingLibrary) {
            libraryRepository.hasCompletedInitialPull = true
            Log.d(TAG, "Skipping FoxTv library pull for profile $profileId because a tracking library provider is active")
            return true
        }
        ...
        return try {
            ...
            libraryRepository.hasCompletedInitialPull = true
            Log.e(TAG, "Periodic FoxTv library pull failed profile=$profileId", e)
            false
        } finally {
            libraryRepository.isSyncingFromRemote = false
        }
