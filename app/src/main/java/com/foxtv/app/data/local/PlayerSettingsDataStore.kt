...
data class PlayerSettings(
    ...
    // FoxTv ExoPlayer Performance Mode
    val foxTvPerformanceModeEnabled: Boolean = DEFAULT_FOXTV_PERFORMANCE_MODE_ENABLED
) {
    ...
    companion object {
        ...
        const val DEFAULT_FOXTV_PERFORMANCE_MODE_ENABLED = false
    }
}
 ...
 * routes via [com.foxtv.app.core.player.DolbyVisionBaseLayerPolicy].
 *
...
class PlayerSettingsDataStore @Inject constructor(
    ...
    private val foxTvPerformanceModeEnabledKey = booleanPreferencesKey("foxtv_performance_mode_enabled")
    ...
    val playerSettings: Flow<PlayerSettings> = profileManager.activeProfileId.flatMapLatest { pid ->
        factory.get(pid, FEATURE).data.onStart { migrateProfile(pid) }
            ...
            PlayerSettings(
                ...
                foxTvPerformanceModeEnabled = (prefs[foxTvPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_FOXTV_PERFORMANCE_MODE_ENABLED) &&
    ...
    val foxTvPerformanceModeEnabled: Flow<Boolean> = profileManager.activeProfileId.flatMapLatest { pid ->
        factory.get(pid, FEATURE).data.map { prefs ->
            (prefs[foxTvPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_FOXTV_PERFORMANCE_MODE_ENABLED) &&
                    android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O
        }
    }

    suspend fun setFoxTvPerformanceModeEnabled(enabled: Boolean) {
        val actualEnabled = enabled && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O
        store().edit { prefs ->
            prefs[foxTvPerformanceModeEnabledKey] = actualEnabled
            if (actualEnabled) {
    ...
    private fun isNativeMemoryActive(prefs: androidx.datastore.preferences.core.Preferences): Boolean {
        val isEnabled = prefs[foxTvPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_FOXTV_PERFORMANCE_MODE_ENABLED
