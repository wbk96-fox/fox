...
/** OAuth coordinates are injectable so auth behavior can be tested without private build secrets. */
class TraktOAuthConfiguration @Inject constructor() {
    private var clientIdOverride: String? = null
    private var clientSecretOverride: String? = null
    private var redirectUriOverride: String? = null
    ...
    val clientId: String get() = clientIdOverride ?: BuildConfig.TRAKT_CLIENT_ID
    val clientSecret: String get() = clientSecretOverride ?: BuildConfig.TRAKT_CLIENT_SECRET
    val redirectUri: String
        get() = (redirectUriOverride ?: BuildConfig.TRAKT_REDIRECT_URI)
            .ifBlank { "urn:ietf:wg:oauth:2.0:oob" }
}
...
class TraktAuthService @Inject constructor(
    ...
    private val oauth: TraktOAuthConfiguration
) {
    ...
    private fun traktRedirectUri(): String = oauth.redirectUri

    fun hasRequiredCredentials(): Boolean {
        return oauth.clientId.isNotBlank() && oauth.clientSecret.isNotBlank()
    }
    ...
    suspend fun startDeviceAuth(): Result<TraktDeviceCodeResponseDto> {
        ...
        suspend fun requestOnce(): Response<TraktDeviceCodeResponseDto> =
            traktApi.requestDeviceCode(
                TraktDeviceCodeRequestDto(clientId = oauth.clientId)
            )
