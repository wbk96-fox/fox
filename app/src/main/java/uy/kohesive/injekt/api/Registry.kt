...
inline fun <reified R : Any, K : Any> InjektRegistry.addPerThreadPerKeyFactory(
    noinline factoryCalledPerKeyPerThread: (K) -> R,
) = addPerThreadPerKeyFactory(fullType<R>(), factoryCalledPerKeyPerThread)
inline fun <reified R : Any> InjektRegistry.addLoggerFactory(
    noinline factoryByName: (String) -> R,
    noinline factoryByClass: (Class<Any>) -> R,
) = addLoggerFactory(fullType<R>(), factoryByName, factoryByClass)
inline fun <reified T : O, reified O : Any> InjektRegistry.addAlias() =
